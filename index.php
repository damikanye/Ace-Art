...
<!--Javascript section for passing food option data--> 
    <script>
      $('a.btn').click( function(evt){
        ...
        var foodItem = JSON.stringify({
          price : price,
          item : item.trim()
        })
        var customUrl="/checkout.php?item=" + foodItem;
        var currentLocation = window.location.href.lastIndexOf('/');
        var newUrl = window.location.href.slice(0,currentLocation)
        window.location.href = newUrl + customUrl;
      })
    </script>
  </body>
</html>
